...
.controller('page3Ctrl', function($scope, $state, $http) {
	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/getQuestions'
    ...
    $scope.addQ=function(q){
    	...
    	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/addQuestion?'+Object.toparams({username : _user.username, question : q.question})
      	}).success(function(res){
           console.log(res);
            $scope.questions=res;
      	}).error(function(error){
...
.controller('page6Ctrl', function($scope, $state, $http, $ionicPopup) {
	...
	$scope.signIn = function(user) {
    	...
    	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/login?'+Object.toparams({username : user.username, password : user.password})
      	}).success(function(res){
           console.log(res);
           if(res.status){
           		_user=user;
...
.controller('page7Ctrl', function($scope, $state, $http, $ionicPopup) {
	$scope.regist = function(user) {
    	...
    	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/regist?'+Object.toparams({username : user.username, password : user.password})
      	}).success(function(res){
           console.log(res);
           if(res.status){
           		$state.go('page6');
...
.controller('page8Ctrl', function($scope, $stateParams, $state, $http) {
	...
	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/getQuestion?'+Object.toparams({id:$stateParams.id})
      	}).success(function(res){
            $scope.question=res;
      	}).error(function(error){
           console.log(error);
    ...
    $scope.addA=function(q){
    	...
    	$http({
           method : 'GET',
           url : 'http://xiaoxiwang.cn:5000/addAnswer?'+Object.toparams({id:$scope.question.id, username : _user.username, answer : q.answer})
      	}).success(function(res){
           console.log(res);
            $scope.question=res;
      	}).error(function(error){
